...
/// GAPM
	...
	int (*gapm_cmd_cmp_evt)(struct gapm_cmp_evt *param);
	// Event triggered to inform that lower layers are ready
	int (*gapm_device_ready_indication)(void *param);
/* Local device information */
	// Local device name indication event
	int (*gapm_dev_name_indication)(struct gapm_dev_name_ind *param);
	// Local device appearance indication event
	int (*gapm_appearance_indication)(struct gapm_appearance_ind *param);
	// Local device version indication event
	int (*gapm_dev_version_indication)(struct gapm_dev_version_ind *param);
	// Local device BD Address indication event
	int (*gapm_dev_bdaddr_indication)(struct gapm_dev_bdaddr_ind *param);
	// Advertising channel Tx power level
	int (*gapm_dev_adv_tx_power_indication)(struct gapm_dev_adv_tx_power_ind *param);
/* White List */
	// White List Size indication event
	int (*gapm_white_list_size_indication)(struct gapm_white_list_size_ind *param);
/* Air Operations */
	// Advertising or scanning report information event
	int (*gapm_adv_report_indication)(struct gapm_adv_report_ind *param);
	// Name of peer device indication
	int (*gapm_peer_name_indication)(struct gapm_peer_name_ind *param);
/* Privacy update events */
	// Privacy flag value has been updated
	int (*gapm_updated_privacy_indication)(struct gapm_updated_privacy_ind *param);
	// Reconnection address has been updated
	int (*gapm_updated_recon_addr_indication)(struct gapm_updated_recon_addr_ind *param);
/* Security / Encryption Toolbox */
	// Indicate that resolvable random address has been solved
	int (*gapm_addr_solved_indication)(struct gapm_addr_solved_ind *param);
	//  AES-128 block result indication
	int (*gapm_use_enc_block_indication)(struct gapm_use_enc_block_ind *param);
	// Random Number Indication
	int (*gapm_gen_rand_nb_indication)(struct gapm_gen_rand_nb_ind *param);

/// GAPC
	// Command Complete event
	int (*gapc_cmd_cmp_evt)(struct gapc_cmp_evt *param);
/* Connection state information */
	// Indicate that a connection has been established
	int (*gapc_connection_req_indication)(struct gapc_connection_req_ind *param);
	// Indicate that a link has been disconnected
	int (*gapc_disconnect_indication)(struct gapc_disconnect_ind *param);
/* Peer device info */
	// Name of peer device indication
	int (*gapc_peer_name_indication)(struct gapc_peer_name_ind *param);
	// Indication of peer version info
	int (*gapc_peer_version_indication)(struct gapc_peer_version_ind *param);
	// Indication of peer features info
	int (*gapc_peer_features_indication)(struct gapc_peer_features_ind *param);
	// Indication of ongoing connection RSSI
	int (*gapc_conn_rssi_indication)(struct gapc_con_rssi_ind *param);
	// Indication of ongoing connection Channel Map
	int (*gapc_conn_channel_map_indication)(struct gapc_con_channel_map_ind *param);
	// Indication of peer privacy info
	int (*gapc_privacy_indication)(struct gapc_privacy_ind *param);
	// Indication of peer reconnection address info
	int (*gapc_recon_addr_indication)(struct gapc_recon_addr_ind *param);
/* Connection parameters update */
	// Request of updating connection parameters indication
	int (*gapc_conn_param_update_req_indication)(struct gapc_param_update_req_ind *param);
	// Connection parameters updated indication
	int (*gapc_conn_param_updated_indication)(struct gapc_param_updated_ind *param);
/* Bonding procedure */
	// Bonding requested by peer device indication message.
	int (*gapc_bond_req_indication)(struct gapc_bond_req_ind *param);
	// Bonding information indication message
	int (*gapc_bond_info_indication)(struct gapc_bond_ind *param);
/* Encryption procedure */
	// Encryption requested by peer device indication message.
	int (*gapc_encrypt_req_indication)(struct gapc_encrypt_req_ind *param);
	// Encryption information indication message
	int (*gapc_encrypt_info_indication)(struct gapc_encrypt_ind *param);
/* Security request procedure */
	// Security requested by peer device indication message
	int (*gapc_security_indication)(struct gapc_security_ind *param);
/* Signature procedure */
	// Indicate the current sign counters to the application
	int (*gapc_sign_counter_indication)(struct gapc_sign_counter_ind *param);

/// GATT
	// Command Complete event
	int (*gattc_cmd_cmp_evt)(struct gattc_cmp_evt *param);
/* ATTRIBUTE CLIENT */
	// Discovery services indication
	int (*gattc_disc_svc_indication)(struct gattc_disc_svc_ind *param);
	// Discover included services indication
	int (*gattc_disc_svc_incl_indication)(struct gattc_disc_svc_incl_ind *param);
	// Discover characteristic indication
	int (*gattc_disc_char_indication)(struct gattc_disc_char_ind *param);
	// Discovery characteristic descriptor indication
	int (*gattc_disc_char_desc_indication)(struct gattc_disc_char_desc_ind *param);
	// Read response including results from peer device
	int (*gattc_read_indication)(struct gattc_read_ind *param);
	// peer device triggers an event (indication or notification)
	int (*gattc_notify_indication)(struct gattc_event_ind *param); // remote indication or notification
/* ATTRIBUTE SERVER */
	/* Indicate that write operation is requested. */
	// Write command indicated to upper layers.
	int (*gattc_write_cmd_indication)(struct gattc_write_cmd_ind *param);
	/* Indicate that a read operation is requested. */
	// Read command indicated to upper layers.
	int (*gattc_read_cmd_indication)(struct gattc_read_cmd_ind *param);
